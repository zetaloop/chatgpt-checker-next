...
查看 ChatGPT、Codex 和 Grok 的账号、用量与服务信息，并启用一些有趣的功能。
...
3. 打开 ChatGPT / Codex / Grok，页面右侧有一个圆圈，将鼠标光标放上去即可查看模型、额度、服务质量等信息。
...
- **Codex 用量**：Codex 的用量进度条与重置时间，以及积分和重置机会。
...
- **账号地区**：当前账号地区。
- **可用模型**：当前可用模型列表。
- **用量**：当前模型额度、存储占用和自动化数量。
- **开发工具**：启用内部调试工具，可以自定义工具、模型、提示词等。
- **其他小功能**：启用异步聊天、抢先体验模型、假装解锁不可用的模型、假装自己是会员等。
